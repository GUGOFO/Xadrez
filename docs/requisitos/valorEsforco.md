# Matriz de Valor vs. Esforço

A distribuição dos 16 Requisitos Funcionais do escopo verde está organizadas exatamente segundo a estrutura da sua matriz.

---

### Grande Valor x Pequeno Esforço

| ID | Requisito |
| :--- | :--- |
| **RF01** | Exibir Tabuleiro |
| **RF02** | Selecionar Peça |
| **RF03** | Mover Peça |
| **RF04** | Alterar Tempo de Jogo |
| **RF07** | Modificar Cores |
| **RF08** | Alterar Nomes |
| **RF09** | Desistir da Partida |
| **RF10** | Propor Empate |
| **RF11** | Exibir Resultado |

---

### Grande Valor x Grande Esforço

| ID | Requisito |
| :--- | :--- |
| **RF05** | Exibir Histórico de Movimentos |
| **RF13** | Exibir Peças Capturadas |
| **RF14** | Exibir Espaços Seguros |
| **RF15** | Validar Jogadas |

---

### Pequeno Valor x Pequeno Esforço 

| ID | Requisito |
| :--- | :--- |
| **RF06** | Voltar Movimentos |

---

### Pequeno Valor x Grande Esforço
 
| **RF12** | Editar Tabuleiro | 

---

# Figma

<iframe style="border: 1px solid rgba(0, 0, 0, 0.1);" width="800" height="450" src="https://embed.figma.com/board/Uqt76H9NhoaXe69QhDVJx1/Xadrez?node-id=10-2515&embed-host=share" allowfullscreen></iframe>

# Imagens

## Figma Completo

![Figma Completo](../img/requisitos/valorEsforco/valorVsEsforco.png)

## Grande Valor x Pequeno Esforço

![Grande Valor x Pequeno Esforço](../img/requisitos/valorEsforco/GVPE.png)

## Grande Valor x Grande Esforço

![Grande Valor x Grande Esforço](../img/requisitos/valorEsforco/GVGE.png)

## Pequeno Valor x Pequeno Esforço 

![Pequeno Valor x Pequeno Esforço](../img/requisitos/valorEsforco/PVPE.png)

## Pequeno Valor x Grande Esforço

![Pequeno Valor x Grande Esforço](../img/requisitos/valorEsforco/PVGE.png)